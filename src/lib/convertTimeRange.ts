...
export function convertScheduleFormat({
  ...
  // Helper functions - REMOVED time conversion since we're using 24-hour format
  ...
  if (direction === "toAvailable") {
    ...
    const result: AvailableSchedule = {
      available: availableDays
        ...
        .map((day) => ({
          ...
          timeSlots: [
            {
              startTime: formatTime(day.startTime), // Already in 24-hour format
              endTime: formatTime(day.endTime),     // Already in 24-hour format
            },
          ],
        })),
    };
    ...
    inputData.available.forEach((day) => {
      ...
      abbreviated[dayAbbr] = {
        ...
        startTime: formatTime(timeSlot.startTime),  // Already in 24-hour format
        endTime: formatTime(timeSlot.endTime),      // Already in 24-hour format
      };
    });
