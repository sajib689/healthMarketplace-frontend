...
import { ReviewPayload, useCreateReviewMutation } from "@/redux/api/review/reviewApi";

const ProjectDelivery = () => {
    // Extract slug from pathname
    const path = usePathname();
    const slug = path.split("/")[2];
    const [modalAction, setModalAction] = useState<"cancel" | "confirm" | "infoModal" | null>(null);
    ...
    return (
        <div className="w-full bg-white border-gray-200 p-6 space-y-6">
            ...
            <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                    <Image
                        width={48}
                        height={48}
                        src={delivery?.agreement.project.user?.profilePicture || UserPlaceholder.src} // Fallback image
                        alt={`${delivery?.agreement.project.user?.firstName ?? ""} ${delivery?.agreement.project.user?.lastName ?? ""}`}
                    ...
                    <div>
                        <h2 className="font-medium text-gray-900 text-lg">
                            {delivery?.agreement.project.user?.firstName} {delivery?.agreement.project.user?.lastName}
                        </h2>
                        <p className="text-sm text-gray-500">{delivery?.agreement.project.user?.email}</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                <h1 className="text-2xl font-semibold text-gray-900">{delivery?.agreement.project.name}</h1>
                <p className="text-sm text-gray-500 mt-1">Posted {postedTime}</p>
            </div>

            {/* Tags (Skills) */}
            <div className="flex flex-wrap gap-2 border-y py-4">
                {delivery?.agreement.project.skills?.map((skill: string, index: number) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-b py-4 max-w-md">
                <div>
                    <h3 className="text-lg font-semibold">{delivery?.agreement.project.budget}</h3>
                    <p className="text-sm text-gray-500">{delivery?.agreement.project.priceType}</p>
                </div>
                <div>
                    <h3 className="text-lg font-semibold">Deadline</h3>
                    <p className="text-sm text-gray-500">
                        {new Date(delivery?.agreement.project.deadline).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                            year: "numeric",
                        })}
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">{delivery?.agreement.project.category}</p>
                </div>
                {delivery?.agreement.project.experienceLevel && (
                    <div>
                        <h3 className="text-lg font-semibold">Experience Level</h3>
                        <p className="text-sm text-gray-500">{delivery?.agreement.project.experienceLevel}</p>
                    </div>
                )}
            ...
            <div className="space-y-2 border-y py-4">
                ...
                <p className="text-gray-600">{delivery?.agreement.project.scopeOfWork}</p>
            </div>
            <div className="mb-6 p-4 border rounded-lg">
                ...
                <div className="flex items-center gap-4 mb-4">
                    <Image
                        width={80}
                        height={80}
                        src={delivery?.agreement.freelancer.profilePicture || UserPlaceholder.src}
                        alt={`${delivery?.agreement.freelancer.firstName ?? ""} ${delivery?.agreement.freelancer.lastName ?? ""
                            }`}
                        className="w-20 h-20 rounded-full object-cover"
                    />
                    <div>
                        <h2 className="font-medium text-gray-900">
                            {delivery?.agreement.freelancer.firstName} {delivery?.agreement.freelancer.lastName}
                        </h2>
                        <p className="text-sm text-gray-500">
                ...
                <div className="mt-4">
                    ...
                    <p className="text-gray-700">{delivery?.agreement.freelancer.about.bio}</p>
                </div>
                ...
                {
                    user?.id !== delivery?.agreement.freelancer.id && <div className="mt-4 flex items-center gap-4">
                        <div onClick={() => setModalAction("confirm")} className=" cursor-pointer text-center py-2 rounded-full bg-secondary/80 transition-all duration-300 text-white hover:bg-secondary shadow px-4">
                            Confirm
                        </div>

                        <div onClick={() => setModalAction("cancel")} className=" cursor-pointer text-center py-2 rounded-full bg-warning/80 transition-all duration-300 text-white hover:bg-warning shadow px-4">
                            Cancel
                        </div>
                    </div>
                }
                ...
                {
                    user?.id !== delivery?.agreement.freelancer.id ? <Modal
                        isOpen={modalAction ? true : false}
                        onClose={() => setModalAction(null)}
                    >

                        <OrderModal
                            name={`${delivery?.agreement.freelancer.firstName ?? ""} ${delivery?.agreement.freelancer.lastName ?? ""
                                }`}
                            avatar={delivery?.agreement.freelancer.profilePicture || UserPlaceholder.src}
                            title={delivery?.agreement.project.name}
                            deadline={new Date(delivery?.agreement.project.deadline).toLocaleDateString("en-US", {
                                month: "short",
                                day: "numeric",
                                year: "numeric",
                            })}
